@SET  logrartitle=日志文件压缩备份工具 LOG-RAR 0.5

@REM 0.5, 2017-05-23, same.
@REM   修正：处理当天日志文件清单 log-today-fmt.lst 时使用GOTO语句会直接终止循环的错误(改为使用skip变量判断)。
@REM 0.4, 2017-04-02, same.
@REM   新增：支持配置当天日志文件清单 log-today-fmt.lst，处理 log-rar.lst 时如匹配会跳过不处理(注：y、m、d用!括起，不能用%)。
...
FOR /F "TOKENS=1" %%f IN (%wd%\log-rar.lst) DO (
    SET skip=0
    IF EXIST %wd%\%%f  (
        IF EXIST %wd%\log-today-fmt.lst (FOR /F "TOKENS=1" %%t IN (%wd%\log-today-fmt.lst) DO (IF "%%f"=="%%t" (
            SET skip=1
        )))
        IF "!skip!" == "0"  (
        ...
        IF EXIST %wd%\log-today-fmt.lst (FOR /F "TOKENS=1" %%t IN (%wd%\log-today-fmt.lst) DO (IF "%year%%%f"=="%%t" (
            SET skip=1
        )))
        IF "!skip!" == "0"  (
